...
import { checkStoredRateLimit, uploadRateLimitOptions } from "@/lib/security/rate-limit";
...
export async function POST(request: Request) {
  try {
    ...
    const limit = await checkStoredRateLimit(`upload:${user.id}:${clientIpFromHeaders(request.headers)}`, uploadRateLimitOptions());
